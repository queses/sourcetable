...
from app.tasks.agent_tasks import run_agent_for_cell_task
    ...
    def _queue_agent_tasks(self, cells: List[Cell], columns: List[TableColumn]) -> None:
        for cell, column in zip(cells, columns):
            if (
                ...
                # DO NOT DO this in production, use a proper background task queue
                asyncio.create_task(run_agent_for_cell_task(cell.id, column.type))
